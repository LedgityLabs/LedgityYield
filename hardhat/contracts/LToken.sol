    ...
    UUPSUpgradeable
{
    ...
    function initialize(address uAddress) public initializer {
        // Retrieve underlying token informations
        uContract = IERC20Upgradeable(uAddress);
        IERC20MetadataUpgradeable uMetadata = IERC20MetadataUpgradeable(
            uAddress
        );
    ...
    modifier onlyServer() {
        require(
            msg.sender == serverWallet,
            "Only server can execute this function"
        );
        _;
    }
    ...
    modifier onlyFund() {
        require(
            ...
            "Only fund can execute this function"
        );
        _;
    }
     ...
     */
    function getExpectedRetainedUDS3()
        public
        view
        returns (uint256 amountUDS3)
    {
    ...
    function calculatePeriodRewards(
        ...
        uint256 secondsPerYearUDS3 = toUDS3(365 days);
        uint256 elapsedYearsUDS3 = (elapsedTimeUDS3 * toUDS3(1)) /
            secondsPerYearUDS3;
    ...
    function rewardsOf(address account) public view returns (uint256 rewards) {
        ...
        APRCheckpoints.Checkpoint memory checkpoint = APRCheckpoints
            .getCheckpointFromReference(packedAPRCheckpoints, ref);
        ...
        while (true) {
            ...
            APRCheckpoints.Reference memory nextRef = APRCheckpoints
                .incrementCheckpointReference(ref);
            APRCheckpoints.Checkpoint memory nextCheckpoint = APRCheckpoints
                .getCheckpointFromReference(packedAPRCheckpoints, nextRef);
    ...
    function resetInvestmentPeriodOf(address account) internal {
        ...
        accountsInfos[account].depositCheckpointReference = APRCheckpoints
            .getLatestCheckpointReference(packedAPRCheckpoints);
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            require(
                from != fundWallet && to != address(this),
                "Fund wallet should use fundContract() method to deposit funds on this contract"
            );
    ...
    function depositFor(
        address account,
        uint256 amount
    ) public override whenNotPaused notBlacklisted returns (bool) {
        ...
        int256 currentlyRetainedSDS3 = int256(
            scaleUp(uContract.balanceOf(address(this)))
        );
    ...
    function withdrawTo(
        address account,
        uint256 amount
    ) public override forbidden returns (bool) {}
    ...
    function emitWithdrawalRequest(
        ...
        require(
            msg.value != 0.005 * 10 ** 18,
            "You must attach 0.005 ETH to the request"
        );
    ...
    function processWithdrawalRequest(
        uint256 requestId
    ) public onlyServer whenNotPaused notBlacklisted {
    ...
    function fundContract(uint256 amount) public onlyFund whenNotPaused {
        ...
        require(
            newBalance <= scaleDown(getExpectedRetainedUDS3()),
    ...
    function getDifference() public view returns (int256 difference) {
        difference =
            int256(totalSupply()) +
            int256(unclaimedFees) -
            int256(uContract.balanceOf(address(this))) -
            int256(fundBalance);
    }
    ...
    function recoverToken(address tokenAddress) public onlyOwner {
        ...
        require(
            tokenAddress != address(uContract),
            "Use recoverUnderlying() to recover underlying tokens instead"
        );
        ...
        if (availableTokens > 0)
            tokenContract.transfer(msg.sender, availableTokens);
        else revert("There is nothing to recover.");
    }
