  ...
  ERC20WrapperUpgradeable
{
   ...
   * @dev This is an override of ERC20Upgradeable.balanceOf() that rewards that have
  ...
  function recoverUnderlying() external onlyOwner {
    ...
    uint256 recoverableAmount = underlying().balanceOf(
      address(this)
    ) - usableUnderlyings;
    ...
    returns (uint256 amount)
  {
    ...
    uint256 expectedRetainedSUD = (totalSupplySUD *
      retentionRateSUD) / SUD.fromInt(100, d);
  ...
  function instantWithdrawal(
    ...
    (
      ...
      uint256 fees
    ) = getWithdrawnAmountAndFees(_msgSender(), amount);
  ...
  function requestWithdrawal(
    ...
    if (
      ldyStaking.tierOf(_msgSender()) >= 2 &&
      withdrawalQueueCursor > 0
    ) {
   ...
   */
  function processBigQueuedRequest(
    uint256 requestId
    ...
    (
      uint256 withdrawnAmount,
      uint256 fees
    ) = getWithdrawnAmountAndFees(request.account, request.amount);
