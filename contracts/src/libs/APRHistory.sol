...
library APRHistory {
   ...
   */
  function eq(
    Reference memory ref1,
    Reference memory ref2
  ) external pure returns (bool) {
    return
      ref1.packIndex == ref2.packIndex &&
      ref1.cursorIndex == ref2.cursorIndex;
  }
  ...
  function newBlankPack(Pack[] storage self) internal {
    ...
    require(
      self.length == 0 || getLatestReference(self).cursorIndex == 3,
      "L7"
    );
