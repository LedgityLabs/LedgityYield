...
interface ILToken is IERC20MetadataUpgradeable {
     ...
     */
    function onlyHighTierInstantWithdrawal() external view returns (bool);

    /**
     * @notice Returns a withdrawal request by its ID
     */
    function withdrawalQueue(uint256) external view returns (WithdrawalRequest memory);

    /**
     * @notice Returns a transfers listener by its index
     */
    function transfersListeners(uint256) external view returns (ITransfersListener);
     ...
     */
    function setWithdrawalFeeInEth(uint256 withdrawalFeeInEth_) external;
