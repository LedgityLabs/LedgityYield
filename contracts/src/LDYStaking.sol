...
contract LDYStaking is BaseUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function stake(
    ...
    require(
      stakeDurationIndex <= stakeDurationInfos.length - 1,
      "Invalid staking period"
    );

    _updateReward(address(0), 0);
    StakeDurationInfo memory stakeDurationInfo = stakeDurationInfos[
      stakeDurationIndex
    ];
    ...
    uint256 weightedStake = (amount * stakeDurationInfo.multiplier) /
      MULTIPLIER_BASIS;
    totalWeightedStake += weightedStake;
    totalStaked += amount;

    stakeRewardToken.safeTransferFrom(
      _msgSender(),
      address(this),
      amount
    );
  ...
  function unstake(
    ...
    require(amount > 0, "amount = 0");
    require(
      userStakingInfo[_msgSender()].length >= stakeIndex + 1,
      "Invalid stakeIndex"
    );
    require(
      block.timestamp >=
    ...
    uint256 currentWeightedStake = (amount * multiplier) /
      MULTIPLIER_BASIS;
    ...
    if (userStakingInfo[_msgSender()][stakeIndex].stakedAmount == 0) {
      ...
      userStakingInfo[_msgSender()][stakeIndex] = userStakingInfo[
        _msgSender()
   ...
   */
  function setStakeDurationForPerks(
    uint256 stakeDurationForPerks_
  ) external onlyOwner {
    stakeDurationForPerks = stakeDurationForPerks_;
  }
   ...
   */
  function setStakeAmountForPerks(
    uint256 stakeAmountForPerks_
  ) external onlyOwner {
    stakeAmountForPerks = stakeAmountForPerks_;
  }
   ...
   * @dev Only callable by owner.
   */
  function pushStakeDurationInfo(
  ...
  function notifyRewardAmount(uint256 amount) external onlyOwner {
    ...
    require(
      rewardRatePerSec <=
        (stakeRewardToken.balanceOf(address(this)) +
          amount -
          totalStaked) /
          rewardsDuration,
  ...
  function earned(
    ...
    StakingInfo memory userInfo = userStakingInfo[account][
      stakeIndex
    ];
    uint256 multiplier = _getMultiplier(userInfo.duration);
    uint256 weightedAmount = (userInfo.stakedAmount * multiplier) /
      MULTIPLIER_BASIS;
   ...
   */
  function getStakeDurationInfo(
    uint256 index
  ) external view returns (StakeDurationInfo memory) {
  ...
  function _updateReward(
    ...
    if (account != address(0)) {
      userStakingInfo[account][stakeIndex].rewards = earned(
        account,
        stakeIndex
      );
      userStakingInfo[account][stakeIndex]
        .rewardPerTokenPaid = rewardPerTokenStored;
    }
  }
