...
contract PreMining is Ownable2Step, Pausable {
   ...
   */
  function eligibleRewardsOf(
    address account
  ) public view returns (uint256) {
    // Compute account's lock weight
    uint256 lockerWeight = accountsLocks[account].amount *
      accountsLocks[account].duration;
  ...
  function lock(
    ...
    uint8 appliedDuration = existingDuration > duration
      ? existingDuration
      : duration;
  ...
  function processUnlockRequests() external onlyOwner {
    ...
    while (processedId < unlockRequests.length) {
      ...
      if (unlockAccount != address(0)) {
        ...
        if (underlyingToken.balanceOf(address(this)) < unlockAmount)
          break;
  ...
  function availableToClaim(
    ...
    if (elapsedTime > vestingInSeconds)
      elapsedTime = vestingInSeconds;

    // Compute total available to claim (proportionally to elapsed time)
    uint256 totalAvailableToClaim = (totalEligibleRewards *
      elapsedTime) / vestingInSeconds;

    // Else return net claimable (available minus already claimed)
    return
      totalAvailableToClaim - accountsLocks[account].claimedRewards;
  }
