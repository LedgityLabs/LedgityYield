...
contract Tests is Test, ModifiersExpectations {
    ...
    function setUp() public {
        ...
        // Deploy LTYStaking
        LTYStaking impl4 = new LTYStaking();
        ERC1967Proxy proxy4 = new ERC1967Proxy(address(impl4), "");
        ltyStaking = LTYStaking(address(proxy4));
        ltyStaking.initialize(
            address(globalOwner),
            address(globalPause),
            address(globalBlacklist),
            address(ltyToken)
        );
        vm.label(address(ltyStaking), "LTYStaking");
    ...
    function test_initialize_2() public {
        console.log("Should properly set global owner, pause, blacklist and lty staking");
        ...
        assertEq(address(tested.ltyStaking()), address(ltyStaking));
    }
    ...
    // ================================
    // === setLTYStaking() function ===
    function testFuzz_setLTYStaking_1(address account, address _contract) public {
        console.log("Should revert if not called by owner");
        ...
        expectRevertOnlyOwner();
        vm.prank(account);
        tested.setLTYStaking(_contract);
    }

    function testFuzz_setLTYStaking_2(address _contract) public {
        console.log("Should change value of ltyStaking");
        tested.setLTYStaking(_contract);
        assertEq(address(tested.ltyStaking()), _contract);
    }
    ...
    function testFuzz_balanceOf_2(
        ...
        // Cap amount to 100T
        depositedAmount = bound(depositedAmount, 0, tested.public_toDecimals(100_000_000_000_000));
    ...
    function testFuzz_recoverUnderlying_3(
        ...
        depositedAmount = uint216(
            bound(depositedAmount, 2, tested.public_toDecimals(100_000_000_000_000))
        );
    ...
    function testFuzz_getExpectedRetained_1(
        ...
        totalSupply = bound(
            totalSupply,
            tested.public_toDecimals(1) + 10000,
            tested.public_toDecimals(100_000_000_000_000)
        );
    ...
    function testFuzz_getExpectedRetained_2(
        ...
        totalSupply = bound(
            totalSupply,
            tested.public_toDecimals(1),
            tested.public_toDecimals(1000_000_000_000_000)
        );
        ...
        assertTrue(difference <= tested.public_toDecimals(1) + 100000);
    }
    ...
    function testFuzz_transferExceedingToFund_2(
        ...
        depositedAmount = bound(
            depositedAmount,
            tested.public_toDecimals(12000),
            tested.public_toDecimals(1000_000_000_000_000)
        );
    ...
    function testFuzz_getWithdrawnAmountAndFees_1(
        ...
        // Set first random APR on LTYStaking contract
        ltyStaking.setAPR(aprUD3);
        ...
        ltyStaking.setTier(2, tier2Amount);

        // Deposit enough $LTY tokens to be eligible to tier 2
        deal(address(ltyToken), account, tier2Amount, true);
        vm.startPrank(account);
        ltyToken.approve(address(ltyStaking), tier2Amount);
        ltyStaking.stake(tier2Amount);
        vm.stopPrank();
    ...
    function testFuzz_instantWithdrawal_5(
        ...
        assertEq(ltyStaking.tierOf(account), 2);
    ...
    function testFuzz_requestWithdrawal_4(
        ...
        requestedAmount = bound(
            ...
            tested.public_toDecimals(100_000_000_000_000) > uint256(type(uint96).max) + 1
                ? tested.public_toDecimals(100_000_000_000_000)
                : uint256(type(uint96).max) + 1
        );
    ...
    function testFuzz_requestWithdrawal_9(
        ...
        deal(address(ltyToken), account2, tier2Amount, true);
        ltyStaking.setTier(2, tier2Amount);
        vm.startPrank(account2);
        ltyToken.approve(address(ltyStaking), tier2Amount);
        ltyStaking.stake(uint216(tier2Amount));
        vm.stopPrank();
        assertEq(ltyStaking.tierOf(account2), 2);
    ...
    function testFuzz_requestWithdrawal_10(
        ...
        assertEq(ltyStaking.tierOf(account2), 0);
