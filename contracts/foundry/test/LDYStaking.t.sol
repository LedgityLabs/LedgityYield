...
contract Tests is Test, ModifiersExpectations {
    ...
    // @dev 100T in UDX60x18 format
    uint256 constant MAX_SUPPLY_UD60x18 = 75_000_000 * 10 ** 18;
    ...
    function testFuzz_recoverLDY_3(uint16 aprUD7x3, uint256 fueledAmount, uint256 stakedAmount) public {
        ...
        fueledAmount = uint216(bound(fueledAmount, 1, MAX_SUPPLY_UD60x18));
        ...
        stakedAmount = uint216(bound(stakedAmount, 1, MAX_SUPPLY_UD60x18));
    ...
    function testFuzz__getLockDurationIncrease_2(
        ...
        // Ensure amount increase rate is in [1%, 100%]
        growthRateUD7x3 = bound(growthRateUD7x3, 1 * 10 ** 3, 100 * 10 ** 3);

        // Exclude tiny amount increases and tiny initial amounts to prevent assertion failures because of precision loss. [1%,]
        initialAmount = bound(initialAmount, 100, MAX_SUPPLY_UD60x18);
        ...
        // Convert growth rate to SUD
        uint256 growthRateSUD = SUD.fromRate(growthRateUD7x3, 18);

        // Compute added amount
        uint256 initialAmountSUD = SUD.fromAmount(initialAmount, 18);
        uint256 addedAmountSUD = (initialAmountSUD * growthRateSUD) / SUD.fromInt(100, 18);
        uint256 addedAmount = SUD.toAmount(addedAmountSUD, 18);

        // Compute expected lock end increase
        uint256 stakeLockDurationSUD = SUD.fromInt(tested.stakeLockDuration(), 18);
        uint256 expectedLockEndIncreaseSUD = (stakeLockDurationSUD * growthRateSUD) /
            SUD.fromInt(100, 18);

        // Assert the exepcted lock end increase fits in a uint40
        uint256 _expectedLockEndIncrease = SUD.toInt(expectedLockEndIncreaseSUD, 18);
        assertLe(_expectedLockEndIncrease, type(uint40).max);

        // Convert expected lock end increase to uint40
        uint40 expectedLockEndIncrease = uint40(_expectedLockEndIncrease);
    ...
    function testFuzz__getLockDurationIncrease_3(
        ...
        initialAmount = uint216(bound(initialAmount, 100, MAX_SUPPLY_UD60x18));

        // Ensure added amount is greater than initial amount
        addedAmount = uint216(bound(addedAmount, initialAmount, MAX_SUPPLY_UD60x18));
    ...
    function testFuzz_unlock_4(uint16 aprUD7x3, uint216 amount) public {
        ...
        amount = uint216(bound(amount, 1, MAX_SUPPLY_UD60x18));
    ...
    function testFuzz_unlock_6(uint16 aprUD7x3, uint216 amount, uint32 unlockFeesRateUD7x3) public {
        ...
        amount = uint216(bound(amount, 10000, MAX_SUPPLY_UD60x18));

        // Ensure unlock fees rate never exceeds 100%
        unlockFeesRateUD7x3 = uint32(bound(unlockFeesRateUD7x3, 0, 100 * 10 ** 3));
    ...
    function testFuzz_stake_4(uint16 aprUD7x3, uint216 accountBalance, uint216 investedAmount) public {
        ...
        investedAmount = uint216(bound(investedAmount, 1, MAX_SUPPLY_UD60x18));
    ...
    function test_stake_5(uint16 aprUD7x3, uint216 amount, uint256 duration) public {
        ...
        // Cap invested amount to 100T
        amount = uint216(bound(amount, 2, MAX_SUPPLY_UD60x18));
    ...
    function testFuzz_unstake_4(uint16 aprUD7x3, uint216 accountStake, uint216 withdrawnAmount) public {
        ...
        withdrawnAmount = uint216(bound(withdrawnAmount, 2, MAX_SUPPLY_UD60x18));
    ...
    function testFuzz_claim_5(uint16 aprUD7x3, uint216 accountStake, uint40 investmentDuration) public {
        ...
        // Stake again to indirectly trigger _beforeInvestmentChange and compound rewards in virtual balance
    ...
    function testFuzz_setTier_4(uint8 currTier, uint256 currAmount, uint256 nextAmount) public {
        ...
        currAmount = bound(currAmount, 2, MAX_SUPPLY_UD60x18);
    ...
    function testFuzz_setTier_5(uint8 currTier, uint256 currAmount, uint256 prevAmount) public {
        ...
        prevAmount = bound(currAmount, 2, MAX_SUPPLY_UD60x18);
