...
contract Tests is Test, ModifiersExpectations {
    ...
    function testFuzz__getLockDurationIncrease_2(
        ...
        amountIncreaseUD7x3 = bound(amountIncreaseUD7x3, 1 * 10 ** 3, 100 * 10 ** 3);
        ...
        uint256 amountIncreaseSUD = SUD.fromRate(amountIncreaseUD7x3, 18);
        uint256 addedAmountSUD = (SUD.fromAmount(initialAmount, 18) * amountIncreaseSUD) /
            SUD.fromInt(100, 18);
        uint256 addedAmount = SUD.toAmount(addedAmountSUD, 18);

        // Compute expected lock end increase
        uint256 expectedLockEndIncreaseSUD = (SUD.fromInt(tested.stakeLockDuration(), 18) *
            amountIncreaseSUD) / SUD.fromInt(100, 18);
        uint40 expectedLockEndIncrease = uint40(SUD.toInt(expectedLockEndIncreaseSUD, 18));
