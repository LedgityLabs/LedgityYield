...
contract Tests is Test {
  ...
  function testFuzz_fromAmount_1(
    ...
    nAmount = bound(
      nAmount,
      0,
      type(uint256).max / 10 ** missingDecimals
    );

    // Assert that the result is input converted to UD71x6
    assertEq(
      SUD.fromAmount(nAmount, decimals),
      nAmount * 10 ** missingDecimals
    );
  }

  function testFuzz_fromAmount_2(
    uint8 decimals,
    uint256 nAmount
  ) public {
    ...
    nAmount = bound(
      nAmount,
      type(uint256).max / 10 ** missingDecimals + 1,
      type(uint256).max
    );
  ...
  function testFuzz_fromAmount_4(
    uint8 decimals,
    uint256 nAmount
  ) public {
    console.log("Should revert on overflow when decimals >=3");
  ...
  function testFuzz_toAmount_1(uint8 decimals, uint256 nSUD) public {
    ...
    assertEq(
      SUD.toAmount(nSUD, decimals),
      nSUD / 10 ** missingDecimals
    );
  }
  ...
  function testFuzz_fromRate_4(
    ...
    nUD7x3 = bound(
      nUD7x3,
      type(uint256).max / 10 ** decimals + 1,
      type(uint256).max
    );
  ...
  function testFuzz_fromInt_4(uint8 decimals, uint256 n) public {
    ...
    n = bound(
      n,
      type(uint256).max / 10 ** (decimals + 3) + 1,
      type(uint256).max
    );
