...
contract Tests is Test, ModifiersExpectations {
  ...
  address payable withdrawerWallet =
    payable(address(bytes20("withdrawerWallet")));
  address payable fundWallet =
    payable(address(bytes20("fundWallet")));
  ...
  function test_constructor_1() public {
    console.log(
      "Should revert if minLockDuration is not at least 1 month"
    );
  ...
  function testFuzz_constructor_3(
    ...
    maxLockDuration = uint8(
      bound(maxLockDuration, minLockDuration, type(uint8).max)
    );
    ...
    assertEq(
      address(instance.underlyingToken()),
      address(underlyingToken)
    );
    assertEq(
      address(instance.underlyingToken()),
      address(instance.lToken().underlying())
    );
    ...
    assertEq(
      instance.maxWeight(),
      lockedHardCap * uint256(maxLockDuration)
    );
  }
  ...
  function testFuzz_setLDYToken_1(
    address account,
  ...
  function test_startClaimPhase_6(uint32 time) public {
    console.log(
      "Should properly set the Claim phase start timestamp"
    );
  ...
  function testFuzz_startRecoveryPhase_3(uint32 time) public {
    ...
    uint256 vestingInSecond = tested.vestingDuration() *
      oneMonthInSeconds;
    time = uint32(
      bound(time, 0, vestingInSecond + threeMonthsInSecond)
    );
  ...
  function testFuzz_startRecoveryPhase_4(uint32 time) public {
    ...
    uint256 vestingInSecond = tested.vestingDuration() *
      oneMonthInSeconds;
    uint256 maxLockInSecond = tested.maxLockDuration() *
      oneMonthInSeconds;
    time = uint32(
      bound(
        time,
  ...
  function testFuzz_startRecoveryPhase_5(uint32 time) public {
    console.log(
      "Should properly set hasRecoveryPhaseStarted to true"
    );
  ...
  function test_recoverERC20_2() public {
    console.log(
      "Should revert if Recovery phase has not started yet"
    );
  ...
  function testFuzz_recoverERC20_3(
    ...
    console.log(
      "Should revert if there is not enough tokens to recover"
    );
  ...
  function testFuzz_recoverERC20_4(
    ...
    console.log(
      "Should properly transfer recovered tokens to owner else"
    );
    ...
    assertEq(
      ldyToken.balanceOf(address(tested)),
      availableAmount - recoveredAmount
    );
  ...
  function testFuzz_eligibleRewardsOf_1(
    ...
    lockDuration = uint8(
      bound(lockDuration, minLockDuration, maxLockDuration)
    );
    ...
    if (instance.maxWeight() > 0) {
      ...
      uint256 appliedLockerWeight = (instance.maxWeight() *
        eligibleRewards) / maxDistributedLDY;
  ...
  function test_eligibleRewardsOf_2(
    ...
    // Cap locked amount to 100T (prevents overflow)
    lockAmount = uint240(
      bound(lockAmount, 1 * 10 ** lToken.decimals(), lockedHardCap)
    );
    ...
    if (eligibleRewards > 0) {
      ...
      uint256 appliedRefWeight = (maxDistributedLDY * lockWeight) /
        eligibleRewards;
  ...
  function testFuzz_eligibleRewardsOf_3(
    ...
    if (instance.maxWeight() > 0) {
      ...
      uint256 lockWeight = lockAmount * lockDuration;
      uint256 appliedMaxDistributedLDY = (eligibleRewards *
        instance.maxWeight()) / lockWeight;

      // Compute difference between applied max ditributed LDY amount and expected one
      uint256 weightDiff = appliedMaxDistributedLDY >
        instance.maxDistributedLDY()
  ...
  function testFuzz_lock_4(
    uint256 lockedHardCap,
    uint240 lockAmount,
    uint8 lockDuration
  ) public {
    console.log(
      "Should revert if deposited amount makes exceeding the hardcap"
    );
  ...
  function testFuzz_lock_5(
    ...
    console.log(
      "Should revert if account has already unlocked in the past"
    );
    ...
    lockAmount = uint240(
      bound(lockAmount, 1, uint256(LOCKED_HARD_CAP))
    );
  ...
  function testFuzz_lock_7(
    ...
    // Ensure lockAmount is at least equal to 1
    lockAmount = uint240(
      bound(lockAmount, 1, uint256(LOCKED_HARD_CAP) / 2)
    );
    ...
    (uint240 lockedAmountBefore, , , , ) = tested.accountsLocks(
      locker
    );
    ...
    (uint240 lockedAmountAfter1, , , , ) = tested.accountsLocks(
      locker
    );
    ...
    // Assert that lock amount of locker has increased by lockAmount
    (uint240 lockedAmountAfter2, , , , ) = tested.accountsLocks(
      locker
    );
    assertEq(lockedAmountAfter2, lockAmount * 2);
  }

  function testFuzz_lock_8(
    address locker,
    uint240 lockAmount,
    uint8 lockDuration
  ) public {
    console.log(
      "Should increase the total locked amount by the locked amount"
    );
  ...
  function test_lock_9(
    address locker,
    uint240 lockAmount,
    uint8 lockDuration
  ) public {
    console.log(
      "Should apply new duration if greater than current one, else keep current one"
    );
    ...
    // Ensure lockAmount is at least equal to 1
    lockAmount = uint240(
      bound(lockAmount, 1, uint256(LOCKED_HARD_CAP) / 3)
    );

    // Ensure lock duration is at least equal to 2 and lower than max lock duration
    lockDuration = uint8(
      bound(lockDuration, 2, MAX_LOCK_DURATION - 1)
    );
    ...
    (, uint8 durationBefore, , , uint40 lockEndBefore) = tested
      .accountsLocks(locker);
    ...
    (, uint8 durationAfter1, , , uint40 lockEndAfter1) = tested
      .accountsLocks(locker);
    assertEq(durationAfter1, lockDuration);
    assertEq(
      lockEndAfter1,
      uint40(
        block.timestamp + uint256(lockDuration) * 30 * 24 * 60 * 60
      )
    );
    ...
    (, uint8 durationAfter2, , , uint40 lockEndAfter2) = tested
      .accountsLocks(locker);
    ...
    (, uint8 durationAfter3, , , uint40 lockEndAfter3) = tested
      .accountsLocks(locker);
  ...
  function testFuzz_lock_10(
    address locker,
    uint240 lockAmount,
    uint8 lockDuration
  ) public {
    console.log(
      "Should deposit amount to L-Token contract if amount > 0"
    );
    ...
    // Ensure lockAmount is at least equal to 1
    lockAmount = uint240(
      bound(lockAmount, 0, uint256(LOCKED_HARD_CAP))
    );

    // Ensure lock duration is at least equal to 1 and 2 lower than max lock duration
    lockDuration = uint8(
      bound(lockDuration, 1, MAX_LOCK_DURATION - 2)
    );
  ...
  function testFuzz_instantUnlock_7(
    ...
    console.log(
      "Should withdraw and transfer unlocked underlying tokens to locker"
    );
  ...
  function testFuzz_requestUnlock_6(
    ...
    console.log(
      "Should append locker's address to unlock requests queue"
    );
  ...
  function testFuzz_requestUnlock_7(
    ...
    console.log(
      "Should revert if not enough processing fees are attached"
    );
  ...
  function testFuzz_requestUnlock_8(
    ...
    console.log(
      "Should revert if too much processing fees are attached"
    );
  ...
  function testFuzz_requestUnlock_9(
    ...
    console.log(
      "Should create a withdrawal request on L-Token contract"
    );
    ...
    (address requestAccount, uint96 requestAmount) = lToken
      .withdrawalQueue(0);
  ...
  function test_processUnlockRequests_2() public {
    console.log(
      "Shouldn't change any state and silently end if queue is empty"
    );
  ...
  function test_processUnlockRequests_8(
    ...
    console.log(
      "Should properly increase queue cursor to next request to be processed"
    );
  ...
  function test_processUnlockRequests_9(
    uint240 lockAmount,
    uint8 lockDuration
  ) public {
    console.log(
      "Should never revert from out of gas and properly end instead"
    );
    ...
    // Ensure lock end is in the past
    (, , , , uint40 lockEndTimestamp) = tested.accountsLocks(
      address(1)
    );
    vm.warp(lockEndTimestamp + 1);
  ...
  function test_availableToClaim_1(
    ...
    console.log(
      "Should properly apply elasped time since Claim phase start"
    );
    ...
    elapsedTime = uint32(
      bound(
        elapsedTime,
        0,
        tested.vestingDuration() * secondsInOneMonth
      )
    );
    ...
    if (tested.eligibleRewardsOf(locker) > 0) {
      ...
      uint256 vestingInSeconds = uint256(tested.vestingDuration()) *
        secondsInOneMonth;
      uint256 appliedElapsedTime = (availableRewards *
        vestingInSeconds) / tested.eligibleRewardsOf(locker);
  ...
  function test_availableToClaim_2(
    ...
    console.log(
      "Should properly apply total eligible rewards of locker"
    );
    ...
    if (elapsedTime > 0) {
      ...
      uint256 vestingInSeconds = uint256(tested.vestingDuration()) *
        secondsInOneMonth;
      uint256 appliedEligibleRewards = (availableRewards *
        vestingInSeconds) / elapsedTime;

      // Compute difference between applied and expected elapsed time
      uint256 eligibleRewardsDiff = appliedEligibleRewards >
  ...
  function test_availableToClaim_3(
    ...
    if (availableRewards > 0) {
      ...
      uint256 appliedVestingSeconds = (elapsedTime *
        tested.eligibleRewardsOf(locker)) / availableRewards;
      uint256 appliedVestingDuration = appliedVestingSeconds /
        secondsInOneMonth;

      // Compute difference between applied and expected vesting duration
      uint256 vestingDurationDiff = appliedVestingDuration >
        tested.vestingDuration()
  ...
  function testFuzz_claimRewards_3(
    ...
    console.log(
      "Should increase account's claimed rewards by the claimed amount"
    );
    ...
    (, , , uint216 claimedRewardsBefore, ) = tested.accountsLocks(
      locker
    );
    ...
    (, , , uint216 claimedRewardsAfter, ) = tested.accountsLocks(
      locker
    );
