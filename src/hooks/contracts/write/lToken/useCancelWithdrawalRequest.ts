...
import {
  ExecuteReturn,
  TransactionStatus,
  useWriteLTokenCancelWithdrawalRequest,
} from "@/types";
import { Address, Hash } from "viem";

export type ParamsCancelWithdrawalRequest = {
  requestId: bigint;
};
...
type Instance = {
  writeContract: ReturnType<
    typeof useWriteLTokenCancelWithdrawalRequest
  >["writeContractAsync"];
  hash: Hash | undefined;
  error: Error | null;
  chainId: number;
...
function formatParams(params: ParamsCancelWithdrawalRequest): FormattedParams {
  return {
...
function checkParams(
  params: ParamsCancelWithdrawalRequest,
): string | undefined {
...
export const configCancelWithdrawalRequest = {
  makeInstance,
  checkParams,
  execute,
};
