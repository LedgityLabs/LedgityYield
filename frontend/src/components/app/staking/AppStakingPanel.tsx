          ...
          >
            <Slider.Track className="bg-border relative flex items-center grow rounded-full h-1">
              {StakeDurations.map((duration, index) => {
                ...
                const position = (index / (StakeDurations.length - 1)) * 100;
                return (
                  <span
                    key={duration}
                    className="absolute bg-gray-500 w-2 h-2 rounded-full text-sm"
                    style={{ left: `calc(${position}% - 4px)` }}
                  >
                    <span className="absolute left-1/2 -translate-x-1/2 text-sm font-semibold text-gray-500 -bottom-6">
                      {duration}
                    </span>
                  </span>
                );
...
export default AppStakingPanel;