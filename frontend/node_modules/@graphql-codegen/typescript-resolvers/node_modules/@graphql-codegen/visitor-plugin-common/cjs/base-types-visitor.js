...
class BaseTypesVisitor extends base_visitor_js_1.BaseVisitor {
    ...
    get scalarsDefinition() {
        ...
        const allScalars = Object.keys(this.config.scalars).map(scalarName => {
            ...
            const comment = scalarType?.astNode && scalarType.description ? (0, utils_js_1.transformComment)(scalarType.description, 1) : '';
            const { scalar } = this._parsedConfig.declarationKind;
            return (comment +
                (0, utils_js_1.indent)(`${scalarName}: { input: ${inputScalarValue}${this.getPunctuation(scalar)} output: ${outputScalarValue}${this.getPunctuation(scalar)} }`));
        });
    ...
    get directiveArgumentAndInputFieldMappingsDefinition() {
        ...
        for (const [directiveName, parsedMapper] of directiveEntries) {
            ...
            const comment = directiveType?.astNode && directiveType.description ? (0, utils_js_1.transformComment)(directiveType.description, 1) : '';
