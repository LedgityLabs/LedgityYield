...
exports.SelectionSetToObject = void 0;
...
class SelectionSetToObject {
    ...
    _collectInlineFragments(parentType, nodes, types) {
        if ((0, graphql_1.isListType)(parentType) || (0, graphql_1.isNonNullType)(parentType)) {
        ...
        if ((0, graphql_1.isObjectType)(parentType)) {
            for (const node of nodes) {
                ...
                const { fields, inlines, spreads } = (0, utils_js_1.separateSelectionSet)(node.selectionSet.selections);
                ...
                if ((0, graphql_1.isObjectType)(typeOnSchema)) {
                ...
                else if ((0, graphql_1.isInterfaceType)(typeOnSchema) && parentType.getInterfaces().includes(typeOnSchema)) {
        ...
        else if ((0, graphql_1.isInterfaceType)(parentType)) {
            const possibleTypes = (0, utils_js_1.getPossibleTypes)(this._schema, parentType);
            for (const node of nodes) {
                const schemaType = node.typeCondition ? this._schema.getType(node.typeCondition.name.value) : parentType;
                const { fields, inlines, spreads } = (0, utils_js_1.separateSelectionSet)(node.selectionSet.selections);
                const spreadsUsage = this.buildFragmentSpreadsUsage(spreads);
                if ((0, graphql_1.isObjectType)(schemaType) && possibleTypes.find(possibleType => possibleType.name === schemaType.name)) {
                ...
                else if ((0, graphql_1.isInterfaceType)(schemaType) && schemaType.name === parentType.name) {
                    ...
                    for (const possibleType of possibleTypes) {
                        ...
                        if ((0, graphql_1.isTypeSubTypeOf)(this._schema, possibleType, fragmentSpreadType)) {
                            this._appendToTypeMap(types, possibleType.name, fields);
                            this._appendToTypeMap(types, possibleType.name, spreadsUsage[possibleType.name]);
                        }
                    }
        ...
        else if ((0, graphql_1.isUnionType)(parentType)) {
            ...
            for (const node of nodes) {
                ...
                if ((0, graphql_1.isObjectType)(schemaType) && possibleTypes.find(possibleType => possibleType.name === schemaType.name)) {
                    ...
                    const possibleInterfaceTypes = (0, utils_js_1.getPossibleTypes)(this._schema, schemaType);
                    for (const possibleType of possibleTypes) {
    ...
    flattenSelectionSet(selections, parentSchemaType) {
        ...
        if (fieldNodes.length) {
            inlineFragmentSelections.push(this._createInlineFragmentForFieldNodes(parentSchemaType ?? this._parentSchemaType, fieldNodes));
        }
        this._collectInlineFragments(parentSchemaType ?? this._parentSchemaType, inlineFragmentSelections, selectionNodesByTypeName);
    ...
    _buildGroupedSelections(parentName) {
        ...
        const possibleTypes = (0, utils_js_1.getPossibleTypes)(this._schema, this._parentSchemaType);
        ...
        if (!this._config.mergeFragmentTypes || this._config.inlineFragmentTypes === 'mask') {
            const grouped = possibleTypes.reduce((prev, type) => {
                ...
                const { incrementalNodes, selectionNodes, fragmentSpreads } = allNodes.reduce((acc, node) => {
                    if ('fragmentDirectives' in node && (0, utils_js_1.hasIncrementalDeliveryDirectives)(node.fragmentDirectives)) {
        ...
        const grouped = possibleTypes.reduce((prev, type) => {
            ...
            const { typeInfo, fields, dependentTypes: subDependentTypes, } = this.buildSelectionSet(schemaType, selectionNodes, {
                parentFieldName: this.buildParentFieldName(typeName, parentName),
            });
            dependentTypes.push(...subDependentTypes);
    ...
    buildSelectionSet(parentSchemaType, selectionNodes, options) {
        ...
        for (const selectionNode of selectionNodes) {
                    ...
                    if (selectionNode.selectionSet) {
                        ...
                        if (linkFieldNode) {
                            linkFieldNode = {
                                ...
                                field: {
                                    ...linkFieldNode.field,
                                    selectionSet: (0, utils_js_1.mergeSelectionSets)(linkFieldNode.field.selectionSet, selectionNode.selectionSet),
                                },
                            };
                        }
                        else {
            ...
            if (parentSchemaType.name === selectionNode.onType ||
                ...
                ((0, graphql_1.isUnionType)(fragmentType) &&
        ...
        const linkFields = [];
        const linkFieldsInterfaces = [];
        for (const { field, selectedFieldType } of linkFieldSelectionSets.values()) {
            const realSelectedFieldType = (0, plugin_helpers_1.getBaseType)(selectedFieldType);
    ...
    transformSelectionSet(fieldName) {
        const possibleTypesList = (0, utils_js_1.getPossibleTypes)(this._schema, this._parentSchemaType);
        const possibleTypes = possibleTypesList.map(v => v.name).sort();
        const fieldSelections = [
            ...(0, utils_js_1.getFieldNames)({ selections: this._selectionSet.selections, loadedFragments: this._loadedFragments }),
        ].sort();
        ...
        const objMap = this._processor.typeCache.get(this._selectionSet.loc) ?? new Map();
    ...
    transformSelectionSetUncached(fieldName) {
    ...
    transformFragmentSelectionSetToTypes(fragmentName, fragmentSuffix, declarationBlockConfig) {
        ...
        const subTypes = Object.keys(grouped).flatMap(typeName => {
            ...
            const flatFields = possibleFields.map(selectionObject => {
                if (typeof selectionObject === 'string')
                    return { value: selectionObject };
                return { value: selectionObject.union.join(' | '), isUnionType: true };
            });
            const content = flatFields.length > 1
        ...
        const fragmentMaskPartial = this._config.inlineFragmentTypes === 'mask' ? ` & { ' $fragmentName'?: '${mergedTypeString}' }` : '';
        // TODO: unify with line 308 from base-documents-visitor
        const dependentTypesContent = this._config.extractAllFieldsToTypes
            ? dependentTypes.map(i => new utils_js_1.DeclarationBlock(declarationBlockConfig)
                .export(true)
                .asKind('type')
                .withName(i.name)
        ...
        return [
            ...dependentTypesContent,
            ...subTypes.map(t => new utils_js_1.DeclarationBlock(declarationBlockConfig)
