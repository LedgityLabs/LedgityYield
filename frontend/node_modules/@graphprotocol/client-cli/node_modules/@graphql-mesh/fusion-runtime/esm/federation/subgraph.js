import { DirectiveLocation, GraphQLDirective, GraphQLSchema, GraphQLString, isObjectType, isOutputType, Kind, parseType, typeFromAST, visit, } from 'graphql';
import { resolveAdditionalResolversWithoutImport } from '@graphql-mesh/utils';
import { astFromField, getDirectiveExtensions, MapperKind, mapSchema, } from '@graphql-tools/utils';
import { HoistField, RenameInputObjectFields, RenameInterfaceFields, RenameObjectFieldArguments, RenameObjectFields, RenameTypes, TransformEnumValues, } from '@graphql-tools/wrap';
import { compareSubgraphNames } from '../utils.js';
export function handleFederationSubschema({ subschemaConfig, realSubgraphNameMap, schemaDirectives, transportEntryMap, additionalTypeDefs, additionalResolvers, stitchingDirectivesTransformer, onSubgraphExecute, }) {
    // Fix name
    ...
    subschemaConfig.schema = mapSchema(subschemaConfig.schema, {
        [MapperKind.TYPE]: type => {
            ...
            const sourceDirective = sourceDirectives?.find(directive => compareSubgraphNames(directive.subgraph, subgraphName));
        ...
        [MapperKind.OBJECT_FIELD]: (fieldConfig, fieldName, typeName, schema) => {
            ...
            if (fieldConfig.args) {
                for (const argName in fieldConfig.args) {
                    ...
                    const argSourceDirective = argSourceDirectives?.find(directive => compareSubgraphNames(directive.subgraph, subgraphName));
    ...
    if (Object.keys(renameArgByFieldByTypeNames).length > 0) {
        transforms.push(new RenameObjectFieldArguments((typeName, fieldName, argName) => {
            const realTypeName = renameTypeNamesReversed[typeName] ?? typeName;
