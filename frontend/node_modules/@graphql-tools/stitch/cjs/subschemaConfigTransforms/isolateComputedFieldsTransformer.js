            ...
            if (isolatedFieldCount && isolatedFieldCount !== Object.keys(objectType.getFields()).length) {
                ...
                for (const fieldName in isolatedFields) {
                    ...
                    for (const type of returnTypes) {
                        ...
                        if (Object.values(subschemaConfig.schema.getTypeMap())
                            .filter(graphql_1.isObjectType) // only objects
                            .filter(t => t !== type) // not this type
                            .filter(t => !isolatedSchemaTypes[t.name]) // not an isolated type
                            .find(t => Object.values(t.getFields()).find(f => (0, graphql_1.getNamedType)(f.type) === type)) // has a field returning this type
                        ) {
                            continue;
                        }
...
function filterIsolatedSubschema(subschemaConfig) {
    ...
    for (const typeName in subschemaConfig.merge) {
        ...
        for (const entryPoint of entryPoints) {
            if (entryPoint.fieldName != null) {
                ...
                if (rootField) {
                    const rootFieldType = (0, graphql_1.getNamedType)(rootField.type);
                    computedFieldTypes[rootFieldType.name] = true;
                    if ((0, graphql_1.isInterfaceType)(rootFieldType)) {
                        (0, utils_1.getImplementingTypes)(rootFieldType.name, subschemaConfig.schema).forEach(tn => {
    ...
    const rootTypeNames = (0, utils_1.getRootTypeNames)(subschemaConfig.schema);
    const typesForInterface = {};
    const filteredSchema = (0, utils_1.pruneSchema)((0, utils_1.filterSchema)({
        ...
        rootFieldFilter: (typeName, fieldName, config) => {
            ...
            if ((0, graphql_1.isAbstractType)(returnType)) {
                const typesForInterface = [
                    returnType.name,
                    ...(0, utils_1.getImplementingTypes)(returnType.name, subschemaConfig.schema),
                ];
        ...
        objectFieldFilter: (typeName, fieldName, config) => {
            ...
            const fieldType = (0, graphql_1.getNamedType)(config.type);
