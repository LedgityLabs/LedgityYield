...
function makeObjectType(node) {
    const config = {
        ...
        interfaces: () => node.interfaces?.map(iface => (0, utils_1.createNamedStub)(iface.name.value, 'interface')) || [],
...
function makeUnionType(node) {
    return new graphql_1.GraphQLUnionType({
        name: node.name.value,
        description: (0, utils_1.getDescription)(node, backcompatOptions),
        types: () => node.types?.map(type => (0, utils_1.createNamedStub)(type.name.value, 'object')) ?? [],
        astNode: node,
    });
}
