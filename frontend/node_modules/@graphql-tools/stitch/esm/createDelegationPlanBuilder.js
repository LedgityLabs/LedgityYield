...
import { extractUnavailableFields, extractUnavailableFieldsFromSelectionSet, leftOverByDelegationPlan, } from '@graphql-tools/delegate';
...
export function createDelegationPlanBuilder(mergedTypeInfo) {
    return memoize5(function delegationPlanBuilder(schema, sourceSubschema, variableValues, fragments, fieldNodes) {
        ...
        if (isAbstractType(typeInSubschema) &&
            ...
            const implementedSubschemas = targetSubschemas.filter(subschema => {
                ...
                return (isAbstractType(typeInTargetSubschema) &&
...
function optimizeDelegationMap(delegationMap, typeName) {
    for (const [subschema, selectionSet] of delegationMap) {
        for (const [subschema2, selectionSet2] of delegationMap) {
            ...
            const unavailableFields = extractUnavailableFieldsFromSelectionSet(subschema2.transformedSchema, 
