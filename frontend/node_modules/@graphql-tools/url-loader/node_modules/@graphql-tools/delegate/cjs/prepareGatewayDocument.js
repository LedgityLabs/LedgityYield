...
function prepareGatewayDocument(originalDocument, transformedSchema, returnType, infoSchema) {
    ...
    return (0, graphql_1.visit)(expandedDocument, (0, graphql_1.visitWithTypeInfo)(typeInfo, {
        [graphql_1.Kind.SELECTION_SET]: node => visitSelectionSet(node, fragmentReplacements, transformedSchema, typeInfo, possibleTypesMap, reversePossibleTypesMap, interfaceExtensionsMap, fieldNodesByType, fieldNodesByField, dynamicSelectionSetsByField, infoSchema, visitedSelections),
    }), 
...
const getExtraPossibleTypesFn = (0, utils_1.memoize2)(function getExtraPossibleTypes(transformedSchema, infoSchema) {
    const extraPossiblesTypesMap = new Map();
    return function getExtraPossibleTypes(typeName) {
        let extraTypesForSubschema = extraPossiblesTypesMap.get(typeName);
        if (!extraTypesForSubschema) {
            extraTypesForSubschema = new Set();
            const gatewayType = infoSchema.getType(typeName);
            const subschemaType = transformedSchema.getType(typeName);
            if ((0, graphql_1.isAbstractType)(gatewayType) && (0, graphql_1.isAbstractType)(subschemaType)) {
...
function visitSelectionSet(node, fragmentReplacements, transformedSchema, typeInfo, possibleTypesMap, reversePossibleTypesMap, interfaceExtensionsMap, fieldNodesByType, fieldNodesByField, dynamicSelectionSetsByField, 
// TODO: Refactor here later for Federation compat
infoSchema, visitedSelections) {
            ...
            if (selection.kind === graphql_1.Kind.INLINE_FRAGMENT) {
                if (selection.typeCondition != null) {
                    ...
                    if (!visitedSelections.has(selection)) {
                        ...
                        if ((0, graphql_1.isObjectType)(typeInSubschema) || (0, graphql_1.isInterfaceType)(typeInSubschema)) {
                    ...
                    for (const possibleTypeName of possibleTypes) {
                        ...
                        if (maybePossibleType != null &&
                            (0, utils_1.implementsAbstractType)(transformedSchema, parentType, maybePossibleType)) {
                ...
                for (const replacement of fragmentReplacements[fragmentName]) {
                    ...
                    if (maybeReplacementType != null &&
                        (0, utils_1.implementsAbstractType)(transformedSchema, parentType, maybeType)) {
                        newSelections.add({
                ...
                if ((0, graphql_1.isAbstractType)(parentType)) {
                    ...
                    const field = fieldMap[fieldName];
                    if (field) {
                        const unavailableFields = (0, extractUnavailableFields_js_1.extractUnavailableFields)(transformedSchema, field, selection, shouldAdd);
