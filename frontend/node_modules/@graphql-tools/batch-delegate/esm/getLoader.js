...
import { getNamedType, GraphQLList, print } from 'graphql';
import { ValueOrPromise } from 'value-or-promise';
import { delegateToSchema, getActualFieldNodes } from '@graphql-tools/delegate';
...
const getLoadersMap = memoize2(function getLoadersMap(_context, _schema) {
    return new Map();
});
...
export function getLoader(options) {
    const { schema, context, info, fieldName = info.fieldName, dataLoaderOptions, fieldNodes = getActualFieldNodes(info.fieldNodes[0]), selectionSet = fieldNodes[0].selectionSet, returnType = info.returnType, } = options;
