...
function createBatchFn(options) {
    ...
    return function batchFn(keys) {
        return new value_or_promise_1.ValueOrPromise(() => (0, delegate_1.delegateToSchema)({
            returnType: new graphql_1.GraphQLList((0, graphql_1.getNamedType)(options.returnType || options.info.returnType)),
...
function getLoader(options) {
    const { schema, context, info, fieldName = info.fieldName, dataLoaderOptions, fieldNodes = (0, delegate_1.getActualFieldNodes)(info.fieldNodes[0]), selectionSet = fieldNodes[0].selectionSet, returnType = info.returnType, } = options;
